...
class ShuffleStars extends Component {
  constructor(props) {
    super(props);
    this.state = {
      removedArticles: [],
 ...
 refusers.on('value', function(dataSnapshot) {
  ...
  sortTeams(dataSnapshot) {
        ...
        for ( var keyScore in currentUsers[key]["scores"] ) {
          if (currentUsers[key]["scores"].hasOwnProperty(keyScore)) {
            if (keyScore in teamsValues) {
              ...
              var starsBlock = currentUsers[key]["scores"][keyScore]["stars"] ;
              for (var keystars in starsBlock) {
                if (starsBlock.hasOwnProperty(keystars)) {
                  teamsValues[keyScore].totalStars += starsBlock[keystars];
                }
              }
              ...
              for (var keystars in starsBlock) {
                if (starsBlock.hasOwnProperty(keystars)) {
                  totalStars += starsBlock[keystars];
                }
              }
              teamsValues[keyScore].totalStars = totalStars;
            }
              console.log("Stars per user "  + teamsValues[keyScore].totalStars);
          }
        }
   ...
   for ( key in teamsValues) {
     ...
     if (teamsValues.hasOwnProperty(key)) {
      ...
      this.state.articles[idx].totalStars = teamsValues[key]["totalStars"];
    }
   }
  ...
  sortShuffle() {
    ...
    const sortDesc = (a, b) => b.totalStars - a.totalStars;
...
ReactMixin(ShuffleStars.prototype, ReactFireMixin);


export default ShuffleStars;
